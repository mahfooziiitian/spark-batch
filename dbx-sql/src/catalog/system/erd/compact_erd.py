    ...
    for src_table, src_cols in table_columns.items():
        for tgt_table, tgt_cols in table_columns.items():
            ...
            for col_name, _ in src_cols:
                if col_name.endswith("_name") or col_name.endswith("_id") or col_name.startswith("parent_") or col_name in ("created_by", "updated_by", "owner"):
                    ref = col_name.replace("_name", "").replace("_id", "").replace("parent_", "")
                    if ref.endswith("y"):
                        ref = ref[:-1] + "ies"
...
def visualize_erd_compact(table_columns, relationships, output_file, cleanup=True):
    ...
    dot.attr(rankdir="LR", fontsize="10", fontname="Arial", splines="true", overlap="false", nodesep="0.3")
    ...
    for rel in relationships:
        if len(rel) == 4:
            ...
            src, tgt, cols = rel if isinstance(rel[2], list) else (rel[0], rel[2], [rel[1]])
            label = ", ".join(cols)
        else:
            continue
...
def main():
    ...
    all_relationships = list({normalize_relation(r) for r in composite_rels + one_way_rels})
    ...
    for rel in all_relationships:
        if len(rel) == 4:
            ...
            print(f"{rel[0]}({', '.join(cols)}) → {rel[1] if isinstance(rel[2], list) else rel[2]}")

    if RENDER_ERD:
        visualize_erd_compact(table_columns, all_relationships, OUTPUT_FILE, cleanup=CLEANUP)
