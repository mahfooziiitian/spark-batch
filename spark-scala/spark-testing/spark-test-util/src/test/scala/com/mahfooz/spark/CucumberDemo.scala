...
@CucumberOptions(
  ...
  glue = Array("com.sysgears", "com.mahfooz.spark"),
...
class TestObjectFactory extends ObjectFactory {
  ...
  override def start(): Unit =
    injector.getInstance(classOf[ScenarioScope]).enterScope()
  override def stop(): Unit =
    injector.getInstance(classOf[ScenarioScope]).exitScope()
  override def getInstance[T](glueClass: Class[T]): T =
    injector.getInstance(glueClass)
...
class TestModule extends AbstractModule {
  ...
  @Provides
  @Singleton
  def session(dataFrames: DataFrames): SparkSession =
    SparkStub.create(dataFrames)
