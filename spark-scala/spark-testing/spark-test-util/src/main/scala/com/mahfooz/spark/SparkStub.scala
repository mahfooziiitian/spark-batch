...
object SparkStub {
  ...
  def create(
      dataFrames: DataFrames = DataFrames.threadLocal,
      tableResolver: Format => TableNameOptionKey = _ => null
  ): SparkSession = {
    val session = Mockito.spy(
      SparkSession
        .builder()
        .master("local[1]")
        .getOrCreate()
    ...
    Mockito
      .doReturn(
        createDataFrameReader(session, dataFrames, tableResolver),
        Array(): _*
      )
      .when(session)
      .read
    ...
    Mockito
      .doAnswer(new Answer[DataFrame]() {
        override def answer(invocation: InvocationOnMock): DataFrame = {
          val (format, tableOrPath) =
            getFormatAndTable(testReader, tableResolver, invocation)

          stubDataFrameMethods(
            dataFrames.read(format, tableOrPath),
            dataFrames,
            tableResolver
          )
        }
  ...
  private def createDataFrameWriter(
      originalWriter: DataFrameWriter[_],
      dataFrames: DataFrames,
      tableResolver: Format => TableNameOptionKey
  ) = {
    ...
    Mockito
      .doAnswer(new Answer[Unit]() {
        override def answer(invocation: InvocationOnMock): Unit = {
          val (format, tableOrPath) =
            getFormatAndTable(testWriter, tableResolver, invocation)

          dataFrames
            .write(format, tableOrPath, getPrivateField(testWriter)("df"))
        }
      })
      .when(testWriter)
      .save()
  ...
  private def stubDataFrameMethods(
      ds: DataFrame,
      dataFrames: DataFrames,
      tableResolver: Format => TableNameOptionKey
  ): DataFrame = {
    val dfStub = Mockito.spy(ds)
    classOf[DataFrame].getMethods
      .filter { method =>
        method.getReturnType.isAssignableFrom(classOf[Dataset[_]])
      }
      .filter { method => !Modifier.isStatic(method.getModifiers) }
      .foreach { method =>
        val doAnswer = Mockito
          .doAnswer(new Answer[Any]() {
            override def answer(invocation: InvocationOnMock): Dataset[_] = {
              stubDataFrameMethods(
                invocation.callRealMethod().asInstanceOf[DataFrame],
                dataFrames,
                tableResolver
              )
            }
        ...
        method.invoke(
          doAnswer,
          method.getParameterTypes.map { clazz =>
            ArgumentMatchers.any(clazz).asInstanceOf[AnyRef]
          }: _*
        )
      }
  ...
  private def getFormatAndTable[T: ClassTag](
      readerOrWriter: T,
      tableResolver: Format => TableNameOptionKey,
      invocation: InvocationOnMock
  ): (String, String) = {
  ...
  private def getPrivateField[T: ClassTag, V](
      obj: T
  )(name: String)(implicit tag: ClassTag[T]): V = {
