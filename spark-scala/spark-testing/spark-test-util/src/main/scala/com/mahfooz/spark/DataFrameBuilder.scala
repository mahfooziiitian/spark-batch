...
case class RowsBuilder(
    private val values: mutable.MutableList[mutable.MutableList[Any]]
) {
...
object DataFrameBuilder {
  implicit class RowValue(value: Any) {
    def unary_! : RowsBuilder = RowsBuilder(
      mutable.MutableList(mutable.MutableList(value))
    )
  }
}
