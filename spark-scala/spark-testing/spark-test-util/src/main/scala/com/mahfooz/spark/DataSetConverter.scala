...
object DataSetConverter {
  implicit class DataTableExtension(dataTable: DataTable) {
    def asDataFrame(
      ...
      spark.createDataFrame(
        dataTable
          .asMaps()
          .map { rows =>
            rows.map { case (key, value) => (key.split(" ")(0), value) }
          }
          .map { rows =>
            namesAndTypes
              .map { case (name, typ) =>
                parseValue(typ, rows.getOrDefault(name, ""))
              }
              .++(defaultValues)
          }
          .map(Row.fromSeq(_)),
    ...
    def asTypeCheckedDataFrame(`class`: Class[_], defaults: String = "")(
      ...
      val tableWithTypes = DataTable.create(
        dataTable
          .asLists()
          .zipWithIndex
      ...
      val defaultsWithTypes = defaults
        .split(",")
        .filter { !_.isEmpty }
        .map { _.trim() }
        .map { addTypeToColumn }
