  ...
  def read(format: String, pathOrTable: String): DataFrame = {
    readDataFrames.getOrElse(
      ...
      throw new IllegalStateException(
        s"Can't find file or table '$pathOrTable' for format '$format'"
      )
    )
  }
...
object DataFrames {
  val threadLocal: DataFrames = new DataFrames() {
    private val threadLocal =
      ThreadLocal.withInitial[DataFrames](new Supplier[DataFrames] {
        override def get(): DataFrames = new DataFrames()
      })

    override def addReadableTable(
        format: String,
        pathOrTable: String,
        dataFrame: DataFrame
    ): Unit = {
