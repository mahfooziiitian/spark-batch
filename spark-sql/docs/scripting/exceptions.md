...
### :material-animation-play: Interactive Visualization — CONTINUE vs EXIT Handler Flow

<div id="viz-scripting-exceptions" class="ts-viz"></div>

Compare what happens to the statements *after* the failing one: a `CONTINUE` handler lets the
block keep going, while an `EXIT` handler immediately leaves the enclosing `BEGIN … END` block.
