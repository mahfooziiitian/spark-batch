...
### :material-animation-play: Interactive Visualization — Block Scope & Shadowing

<div id="viz-scripting-variables" class="ts-viz"></div>

Step through declaring `x` in an outer block, shadowing it in a nested block, and returning
to the outer scope once the inner block ends — watch which value `SELECT x` resolves to at each step.
...
!!! tip "Session Variables (Spark 4.0)"
    ...
    `BEGIN...END` blocks. See [Session Variables](../spark-4/variables/index.md).
