...
### :material-animation-play: Interactive Visualization — Branch Explorer

<div id="viz-scripting-control" class="ts-viz"></div>

Pick a `load_mode` value and see which `IF / ELSEIF / ELSE` branch actually runs.
Spark evaluates conditions top-to-bottom and stops at the first one that is `TRUE`.
