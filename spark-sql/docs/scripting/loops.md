...
### :material-animation-play: Interactive Visualization — Loop Iteration Tracer

<div id="viz-scripting-loops" class="ts-viz"></div>

Switch between `WHILE`, `REPEAT`, `LOOP`, and `FOR` to compare exactly when each
construct checks its condition and how many times the body runs.
