...
!!! note "Expression vs Statement control flow"
    ...
    (`CASE WHEN`, `IF()`, `COALESCE`), see [Conditions & Predicates](../querying/condition/index.md).

______________________________________________________________________

### :material-animation-play: Interactive Visualization — Script Execution Trace

<div id="viz-scripting-overview" class="ts-viz"></div>

Toggle whether pending orders exist and watch the same `BEGIN … END` block take a different
path: the `IF` branch only inserts an audit row when its condition evaluates to `TRUE`.

<script src="../assets/js/scripting-viz.js"></script>
...
!!! tip "Session Variables"
    ...
    see [Session Variables](../spark-4/variables/index.md).
