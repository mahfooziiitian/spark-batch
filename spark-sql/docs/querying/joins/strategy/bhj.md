...
Broadcast Hash Join broadcasts a small input to every executor so Spark can join it with a large dataset without shuffling the large side.
...
1. Spark automatically chooses broadcast join when the smaller table's estimated size is below `spark.sql.autoBroadcastJoinThreshold` (default **10 MB**).
2. The smaller ("broadcast") side is collected to the driver, serialized, and pushed to every executor — no shuffle occurs on that side.
3. The larger ("streamed") side is read partition-by-partition on each executor; each partition is probed against the in-memory hash table built from the broadcast side.
4. Broadcast join supports `=` equi-joins only. `BroadcastHashJoin` is used for supported one-sided join forms such as inner and left/right outer joins, but not full outer joins.
5. If the optimizer's size estimate is wrong (e.g., no statistics), it may choose `SortMergeJoin` even when broadcast would be faster. Use the `BROADCAST` hint to force it.
6. Setting `spark.sql.autoBroadcastJoinThreshold = -1` disables automatic broadcasting; only explicit hints will trigger it.
...
| Broadcast on `NULL` join key | Rows silently dropped | Use `<=>` or filter `NULL`s before join |
...
!!! note "Broadcast distribution"
    Broadcast relations are distributed among executors using the BitTorrent protocol for efficiency.
