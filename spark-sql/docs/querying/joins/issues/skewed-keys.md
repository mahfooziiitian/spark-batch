...
### :material-animation-play: Interactive Visualization — Hot Keys and Salting

<div id="viz-joins-issues-skewed-keys" class="ts-viz"></div>

The first panel highlights the observed hot-key concentration; the second shows how salting preserves row count while redistributing work. AQE skew splitting is enabled separately and only fires once runtime thresholds are actually exceeded.
...
-- These settings enable AQE's skew-join optimization. Whether Spark actually splits a
-- partition depends on the runtime partition size crossing the skew thresholds; this
-- tiny demo is useful for the shape of the data, not for forcing a split by itself.
```
...
| Skew caused by NULL keys                                    | See [Null Key Trap](null-key-trap.md) — often the "hot key" is `NULL` |
...
!!! note "Skew is not the same as a wrong partition count"
    ...
    partition-**count** problem — see [Partition-Count Problems](partition-count.md).
