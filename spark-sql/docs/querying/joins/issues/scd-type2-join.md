...
entirely, covered in [Incomplete Join Conditions](incomplete-join-conditions.md).
...
### :material-animation-play: Interactive Visualization — Choosing the Correct SCD2 Version

<div id="viz-joins-issues-scd-type2-join" class="ts-viz"></div>

The three states here map directly to the verified Spark 4.2 outputs: `is_current` mislabels history, `BETWEEN` double-matches the boundary date, and the half-open interval returns one version per fact.
...
A `NULL` upper bound is the [Null Key Trap](null-key-trap.md) applied to a range
...
### Example 5 — Defense in Depth: Deterministic Ranking When Windows Overlap Anyway

The half-open interval (Example 3) is correct **only if the SCD2 table's validity
windows are truly non-overlapping**. A bad merge/backfill can still produce overlapping
windows for the same `customer_id` — and a half-open predicate alone won't catch it:

```sql
-- Data-quality bug: the Bronze row's valid_to (2024-06-05) overlaps the Gold row's
-- valid_from (2024-06-01) by 4 days, instead of the two meeting exactly.
INSERT INTO dim_customer VALUES
    (1, 'Bronze', DATE'2024-01-01', DATE'2024-06-05', false),  -- should end 2024-06-01
    (1, 'Gold',   DATE'2024-06-01', DATE'9999-12-31', true);

-- Even the "correct" half-open join still double-matches an order dated inside the
-- overlap (e.g. 2024-06-03), because the predicate is satisfied by BOTH rows.
SELECT o.order_id, d.tier, d.valid_from, d.valid_to
FROM orders o
JOIN dim_customer d
  ON o.customer_id = d.customer_id
 AND o.order_date >= d.valid_from
 AND o.order_date <  d.valid_to;
-- Returns two rows: Bronze AND Gold for the same order.
```

The fix is to make the match **deterministic** with an explicit ranking, so overlapping
or duplicate versions can never produce more than one row per fact even when the
dimension data itself is imperfect:

```sql
SELECT order_id, order_date, tier
FROM (
    SELECT o.order_id, o.order_date, d.tier,
           ROW_NUMBER() OVER (
...
`ROW_NUMBER() ... ORDER BY valid_from DESC` deterministically keeps the **most recently
started** matching version and discards any others, converting an at-most-N-way match
into an always-exactly-one-row result. Treat this as a safety net on top of — not a
replacement for — the half-open interval: it protects against upstream data-quality
regressions (overlapping windows, duplicate loads) that the interval predicate alone
can't guarantee against. See [Incomplete Join Conditions](incomplete-join-conditions.md)
for the same `QUALIFY`/`ROW_NUMBER()` pattern applied to "latest as-of" lookups more
generally.
...
| Overlapping/duplicate versions could slip through (data quality)  | Add `ROW_NUMBER() OVER (PARTITION BY fact_key ORDER BY valid_from DESC) = 1` as a deterministic tiebreaker on top of the interval (Example 5) |
...
!!! danger "Half-open intervals are the SCD2 default for a reason"
    ...
    a sentinel via `COALESCE`. See [Range / Non-Equi Join](range-join-pitfalls.md) for
