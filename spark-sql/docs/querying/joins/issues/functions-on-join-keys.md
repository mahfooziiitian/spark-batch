...
*necessary* to reconcile keys (see [Case & Whitespace Mismatch](case-whitespace-mismatch.md)
and [Type Mismatch](type-mismatch.md)), but it carries two hidden costs:
...
### :material-animation-play: Interactive Visualization — Raw Key vs. Wrapped Key

<div id="viz-joins-issues-functions-on-join-keys" class="ts-viz"></div>

This visualization summarizes the plan difference Spark 4.2 actually produced: raw partition keys gave a direct `region = east` partition prune, while `UPPER(region)` degraded into a weaker dynamic-pruning path.
...
match — closely related to [Incomplete Join Conditions](incomplete-join-conditions.md).
