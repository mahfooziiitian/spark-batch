...
[Null Key Trap](null_key_trap.md).
...
- Rows silently vanish from the result with no error at all (when the cast is wrapped in
  ...
  [Null Key Trap](null_key_trap.md), but caused by a failed *cast*, not a literal `NULL`.
...
ORDER BY o.order_id;
-- Result (Spark 4, ANSI enabled): job FAILS with
-- [CAST_INVALID_INPUT] The value '01/25/2024' of the type "STRING" cannot be cast to
-- "DATE" because it is malformed. SQLSTATE: 22018
