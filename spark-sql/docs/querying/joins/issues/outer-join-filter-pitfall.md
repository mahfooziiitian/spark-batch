...
!!! info "Prerequisite concept"
    ...
    [Join Predicate vs. Filter](predicate-vs-filter.md) first.
...
### :material-animation-play: Interactive Visualization — WHERE vs. ON After a LEFT JOIN

<div id="viz-joins-issues-outer-join-filter-pitfall" class="ts-viz"></div>

This before/after demo shows the row-count drop the moment the right-side predicate moves into `WHERE`. It matches the verified Spark 4.2 plans: `Inner` for the bad version, `LeftOuter` for the correct one.
...
In the Spark 4.2 verification run, `EXPLAIN` for Example 1 actually planned an
`Inner` `BroadcastHashJoin`, while Example 3 below stayed `LeftOuter` — a concise way
to prove the `WHERE` clause changed semantics rather than just post-processing rows.
...
| Need to explicitly find unmatched left rows                                                                 | `LEFT JOIN ... WHERE right.key IS NULL` — this is the *one* `WHERE`-after-outer-join pattern that's intentional and correct, per [Null Key Trap](null-key-trap.md) |
