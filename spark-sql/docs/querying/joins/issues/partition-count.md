...
[Skewed Join Keys](skewed-keys.md), where the *count* is fine but a few partitions are
...
### :material-animation-play: Interactive Visualization — AQE Coalescing vs. Fixed 200 Partitions

<div id="viz-joins-issues-partition-count" class="ts-viz"></div>

The tabs summarize the actual local Spark 4.2 behavior: the same tiny shuffle ended with 1 partition under AQE and 200 partitions when AQE was disabled.
...
| A *few* partitions are huge but the count is fine | That's skew, not partition count — see [Skewed Join Keys](skewed-keys.md) (AQE skew-join or salting)                            |
...
!!! tip "Partition count vs. partition skew are different problems"
    ...
    `spark.sql.shuffle.partitions`. **Skew** ([Skewed Join Keys](skewed-keys.md)) is a
