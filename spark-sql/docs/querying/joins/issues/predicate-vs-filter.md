...
documented in detail in [Outer Join Filter Pushdown](outer-join-filter-pitfall.md).
...
### :material-animation-play: Interactive Visualization — Clause Placement Rules

<div id="viz-joins-issues-predicate-vs-filter" class="ts-viz"></div>

Use the inner/outer toggle to see the one rule that matters: for inner joins the result shape stayed the same in Spark 4.2, while outer joins changed as soon as the predicate moved from `ON` to `WHERE`.
...
Both produce the **same** physical join shape in Spark 4.2 — a
...
The optimizer can freely move inner-join predicates between `ON` and `WHERE`, which is
*why* they're interchangeable there — the choice is purely stylistic.
...
| Find left rows that had **no match** (anti-join)                                        | `WHERE right.key IS NULL` | The one intentional `WHERE`-after-outer-join pattern — see [Null Key Trap](null-key-trap.md)                                            |
| Filter an outer join on a **right-side** column and still keep unmatched left rows      | `ON` (never `WHERE`)      | A `WHERE` on a right-side column silently converts the outer join to inner — [Outer Join Filter Pushdown](outer-join-filter-pitfall.md) |
