...
[Accidental Cross Join](cartesian-join.md), there **is** an `ON` clause here — it's just
...
### :material-animation-play: Interactive Visualization — Partial Key vs. Business Key

<div id="viz-joins-issues-incomplete-join-conditions" class="ts-viz"></div>

The toggle shows why adding just *any* extra predicate is not enough. Spark 4.2 verified both the bad fan-out on `product_id` alone and the over-strict zero-row outcome from exact-date equality.
...
- Row counts after the join are inflated, but not necessarily obviously so — the
    ...
    ordinary [Data Explosion](data-explosion.md) rather than a missing predicate.
...
[Range Join — Using Range Condition with Additional Join Conditions](../types/non_equi_join/range_join/point-in-interval.md#9-using-range-condition-with-additional-join-conditions).
...
| ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Lookup/dimension table has multiple historical versions per natural key (SCD Type 2)                       | Add a temporal range condition (`effective_date <= fact_date [AND fact_date < end_date]`) plus `QUALIFY ROW_NUMBER()` if there's no explicit end-date column — see [SCD Type 2 Join Problems](scd-type2-join.md) |
...
!!! tip "This often masquerades as a Data Explosion or Skew issue"
    ...
    expected, they're easy to misdiagnose as [Data Explosion](data-explosion.md) or
    [Skewed Join Keys](skewed-keys.md). The distinguishing question: does the
