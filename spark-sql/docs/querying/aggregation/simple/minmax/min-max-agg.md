...
### :material-animation-play: Interactive Visualization — Ordering Across Types

<div id="viz-minmax-type-order" class="ts-viz"></div>

Compare numeric, string, and date ordering, then contrast aggregate `MIN` / `MAX` with row-level `GREATEST` / `LEAST`.
...
5. **GREATEST / LEAST** — these are *row-level* functions, not aggregates; `GREATEST(a, b, c)` returns the largest value across *columns within a single row*, while `MAX(col)` aggregates across *rows*. In Spark 4.2 they skip `NULL` arguments and return `NULL` only when every argument is `NULL`.
...
7. **`MIN_BY` / `MAX_BY`** — a dedicated, more readable alternative to the `STRUCT` trick: `MAX_BY(order_id, amount)` returns the `order_id` for the row with the largest `amount`, without manually building or unpacking a struct.
8. **Associative and distributive** — unlike `AVG` (see [AVG](../avg/spark-avg.md)), `MIN` / `MAX` are fully associative: `MIN(MIN(partition_1), MIN(partition_2), ...)` always equals the true global minimum. This makes them trivial to merge across partitions/executors with no risk of the pitfalls that affect averaging partial results.
...
ORDER BY order_id;
-- Verified in PySpark 4.2: GREATEST(10, NULL, 5) = 10 and LEAST(10, NULL, 5) = 5; only an all-NULL argument list returns NULL
```
...
### 8 — `MIN_BY` / `MAX_BY`: simpler argmin / argmax

```sql
-- Equivalent to the STRUCT trick in examples 6-7, but reads more naturally
SELECT
    region,
    MAX_BY(order_id, amount) AS order_id_of_max_amount,
    MAX(amount)              AS max_amount,
    MIN_BY(order_id, amount) AS order_id_of_min_amount,
...
ORDER BY region;
-- Result:
-- region | order_id_of_max_amount | max_amount | order_id_of_min_amount | min_amount
-- --------|------------------------|------------|------------------------|------------
-- East    | 6                      | 450.0      | 3                      | 80.0
-- North   | 4                      | 210.0      | 7                      | 90.0
-- West    | 2                      | 340.0      | 5                      | 150.0
```

### 9 — Running (cumulative) MIN / MAX over time
...
SELECT
    ...
    order_date,
    amount,
    MIN(amount) OVER (PARTITION BY region ORDER BY order_date
                       ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_min,
    MAX(amount) OVER (PARTITION BY region ORDER BY order_date
                       ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_max
FROM sales
WHERE amount IS NOT NULL AND region = 'East'
ORDER BY order_date;
-- Result:
-- region | order_date | amount | running_min | running_max
-- --------|------------|--------|-------------|------------
-- East    | 2024-01-15 | 120.0  | 120.0       | 120.0
-- East    | 2024-02-10 | 80.0   | 80.0        | 120.0
-- East    | 2024-03-05 | 450.0  | 80.0        | 450.0
```

### 10 — Subtotal and grand-total extremes with ROLLUP
...
-- Result (excerpt):
-- region | product | min_sale | max_sale
-- --------|---------|----------|----------
-- East    | Gadget  | 450.0    | 450.0
-- East    | Widget  | 80.0     | 120.0
-- East    | NULL    | 80.0     | 450.0    -- subtotal per region
-- ...
-- NULL    | NULL    | 80.0     | 450.0    -- grand total
-- Because MIN/MAX are associative, ROLLUP subtotals and the grand total
-- can be computed directly from the leaf groups without re-scanning `sales`.
```
...
| Full row associated with the min/max value (argmin/argmax) | `MIN/MAX(STRUCT(key_col, other_cols...))` or `MIN_BY/MAX_BY(result_col, key_col)` |
| Running min/max over time                                  | `MIN/MAX(col) OVER (ORDER BY ...)`                                                |
| Subtotals/grand-total extremes per dimension               | `MIN/MAX(col)` with `GROUP BY ROLLUP/CUBE (...)`                                  |
