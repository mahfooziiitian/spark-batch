...
### :material-animation-play: Interactive Visualization — Explicit Grouping Sets

<div id="viz-olap-grouping-sets" class="ts-viz"></div>

Turn grouping sets on and off to see exactly which subtotal rows Spark emits and which `GROUPING_ID()` values appear.
...
5. **Any combination** — unlike `ROLLUP` and `CUBE`, you can mix arbitrary column subsets in any order, including non-hierarchical combinations. Spark emits only the sets you list; omitted sets never appear.
...
!!! tip "Verified with real NULL inputs"
    ...
    In PySpark 4.2, `GROUPING SETS ((region, product), (region), (product), ())` over data containing real `NULL` keys emitted visually similar rows with different grouping bits, including `gid=0`, `gid=1`, `gid=2`, and `gid=3`. For example, `region=NULL, product=NULL` appeared once as a product subtotal (`gid=2`) and again as the grand total (`gid=3`).
...
!!! warning "Selected columns may become `NULL` at some levels"

    You do **not** have to project every grouped column, but any non-aggregated grouping column that you *do* select will be `NULL` in the grouping sets where that column is rolled up. Plan labels and filters around `GROUPING()` / `GROUPING_ID()` instead of `IS NULL` alone.
...
| GROUPING SETS vs UNION ALL       | `GROUPING SETS` usually outperforms equivalent `UNION ALL` queries because it scans the base table once.                                          |
