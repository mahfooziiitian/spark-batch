...
### :material-animation-play: Interactive Visualization — EXPLODE vs OUTER

<div id="viz-filter-lateral-view" class="ts-viz"></div>

Toggle between `explode` and `explode_outer` to see how `NULL` and empty arrays behave. The row counts are based on Spark 4.2 queries against the exact setup shown here.
...
2. **explode drops `NULL` and empty arrays** — rows with `NULL` arrays or `ARRAY()` contribute no output rows.
3. **explode_outer keeps both cases** — `LATERAL VIEW OUTER explode(arr)` preserves `NULL` arrays and empty arrays, emitting a single `NULL` element row for each.
