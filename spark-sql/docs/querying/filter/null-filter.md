...
<div id="viz-filter-null-filter" class="ts-viz"></div>

Switch between `=`, `<=>`, and `IS NULL` to see which rows survive a `WHERE`. The states match Spark 4.2 behavior verified against real queries.
