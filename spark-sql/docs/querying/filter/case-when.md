...
### :material-animation-play: Interactive Visualization — CASE Branch Evaluation

<div id="viz-filter-case-when" class="ts-viz"></div>

Toggle region and amount to see which branch wins. It reflects Spark 4.2's verified first-match behavior and the fact that a boolean `CASE` can be used directly inside `WHERE`.
