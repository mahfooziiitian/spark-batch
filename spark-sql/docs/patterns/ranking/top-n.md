...
!!! danger "Anti-pattern: a single global ORDER BY ... LIMIT N"

    ```sql
    -- BUG: top 3 rows OVERALL, not top 3 per category.
    ...
    A plain `ORDER BY ... LIMIT N` with no `PARTITION BY` sorts and truncates the
    **entire** result set, with no notion of "category" at all. If one category happens
    to dominate revenue, its rows can crowd out every other category's top performers
    completely — the query returns 3 rows total, not 3 per category, and smaller
    categories may not appear at all. Always rank **within** each group with
    `ROW_NUMBER()`/`RANK()` (`PARTITION BY category`) and filter on the rank, as in the
    examples above — `LIMIT` alone has no concept of "per group".
...
| A global `ORDER BY ... LIMIT` gives the wrong rows | Add `PARTITION BY <group>` inside a window function — plain `LIMIT` truncates the whole result, not per group |
