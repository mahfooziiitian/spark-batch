...
!!! tip "Combine with sessionization"
    For per-session funnels (not per-user lifetime), first sessionize the events using the [Sessionization](../sequence/sessionization.md) pattern, then run the funnel analysis within each session.
