...
### How Partial Sketches Merge Across Partitions

`PERCENTILE_APPROX` scales the same way `APPROX_COUNT_DISTINCT` does (see
[Approximate Distinct Counts](../../../querying/aggregation/simple/count/approx-count-distinct.md)):
each partition builds its own local sketch, and only the small sketches — not the raw
rows — move across the shuffle boundary. Verified on Spark 4.2:

```sql
EXPLAIN SELECT endpoint, PERCENTILE_APPROX(latency_ms, 0.95) AS p95
FROM api_requests GROUP BY endpoint;
```

```text
+- ObjectHashAggregate(keys=[endpoint], functions=[percentile_approx(latency_ms, 0.95, ...)])
   +- Exchange hashpartitioning(endpoint, 200)                 <- only sketches shuffle
      +- ObjectHashAggregate(keys=[endpoint], functions=[partial_percentile_approx(latency_ms, 0.95, ...)])
         +- Project [...]
```

`partial_percentile_approx` builds one t-digest per partition per endpoint;
`percentile_approx` (post-shuffle) **merges those digests together**, not the
original latency values — this is what makes the aggregate cheap to run across an
arbitrarily large, multi-partition dataset. Note the operator name is
`ObjectHashAggregate`, not `HashAggregate` — percentile sketches are maintained as
Java objects rather than the primitive-only buffer `HashAggregate` uses.

### Merging/Deduplicating Rows Before a Percentile Changes the Answer

Unlike `COUNT`/`SUM`, a percentile is **frequency-sensitive**: how many times a value
occurs changes where it lands in the distribution. Silently merging/deduplicating
rows before computing `P95`/`P99` — e.g. `DISTINCT`-ing duplicate latency samples, or
collapsing repeated readings from a retried request — throws away that weighting and
returns a different answer, not a more "correct" one. Verified on Spark 4.2 with five
readings, three of which are the same value:

```sql
CREATE OR REPLACE TEMP VIEW dup_test AS
SELECT * FROM VALUES (100), (100), (100), (200), (300) AS t(latency_ms);

SELECT PERCENTILE_APPROX(latency_ms, 0.5) AS p50 FROM dup_test;
-- p50 = 100   -- correct: 3 of 5 samples (60%) are 100, so the median IS 100

SELECT PERCENTILE_APPROX(DISTINCT latency_ms, 0.5) AS p50_distinct FROM dup_test;
-- p50_distinct = 200  -- WRONG for "median latency": treats {100, 200, 300} as
-- equally likely, hiding that 100 was actually the dominant value
```

`PERCENTILE_APPROX(DISTINCT ...)` isn't a bug — it answers a different question
("what's the median of the *distinct values that occurred*", useful for things like
price tiers), but it is the wrong tool for a metric like latency where each row is one
real observation and repeats matter. **Only merge/deduplicate rows ahead of a
percentile calculation when the duplicates are genuinely redundant (e.g. an exact
retry that should count once)** — never merge them just because they look like
duplicate values.

!!! danger "A `dropDuplicates()`/`DISTINCT` upstream of a percentile is a silent correctness bug"

    If an upstream step (a dedup pass, a `MERGE`, a `GROUP BY` collapsing "duplicate"
    rows) removes repeated observations before a `PERCENTILE_APPROX` runs, the
    resulting P95/P99 no longer reflects the true distribution of what happened — it
    reflects the distribution of *distinct values*, which is a different metric. Check
    whether any earlier stage in the pipeline deduplicates on the metric column itself
    (not just a business key) before trusting a percentile computed downstream.
...
| **Sketch merging**               | Per-partition sketches merge across the shuffle (`partial_percentile_approx` → `percentile_approx`); never merges raw rows |
| **Deduplication risk**           | `PERCENTILE_APPROX(DISTINCT col, ...)` (or an upstream dedup) drops frequency weighting and changes the result             |
...
| Capacity planning           | See [Capacity Planning](capacity-planning.md) — P95 usage as "practical peak" |
| Queue / wait time SLAs      | See [Queue Analysis](queue-analysis.md) — P95 wait time breach detection      |
...
- [Capacity Planning](capacity-planning.md) — P95 usage as "practical peak" for scaling decisions
- [Queue Analysis](queue-analysis.md) — P95 wait/total time and SLA breach detection
- [Outlier Detection](../../data_quality/outlier-detection.md) — flagging values beyond percentile thresholds
