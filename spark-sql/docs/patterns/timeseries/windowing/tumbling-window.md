...
### 15-minute tumbling windows for audit traffic

```sql
-- [Databricks] Requires SELECT on system.access.audit
SELECT
    window(event_time, '15 minutes').start AS window_start,
    window(event_time, '15 minutes').end AS window_end,
    service_name,
    COUNT(*) AS event_count
FROM system.access.audit
WHERE event_time >= CURRENT_TIMESTAMP() - INTERVAL 1 DAY
GROUP BY window(event_time, '15 minutes'), service_name
ORDER BY window_start, service_name;
-- Result (illustrative):
-- window_start        | window_end          | service_name | event_count
-- ------------------- | ------------------- | ------------ | -----------
-- 2024-07-02 09:00:00 | 2024-07-02 09:15:00 | notebooks    | 201
-- 2024-07-02 09:00:00 | 2024-07-02 09:15:00 | clusters     | 32
-- 2024-07-02 09:15:00 | 2024-07-02 09:30:00 | notebooks    | 188
```

!!! tip "Tumbling windows create clean operational reporting buckets"

    Fixed, non-overlapping windows are a natural fit for dashboards and SLA
    reports. The same `window(ts, size)` pattern can summarize audit events,
    query starts, job runs, or any other timestamped system-table stream.
