...
graph LR
    S[Staging] --> C{Changed?}
    C -- Yes --> E1["Step 1: Expire current row\nend_date = now, is_current = false"]
    E1 --> E2["Step 2: Insert new version\nstart_date = now, is_current = true"]
    E2 --> U["Step 3: Backfill current values\non ALL history rows\n(Type 1 overwrite)"]
    U --> P["Step 4: Set prev_city\nfrom the expired row\n(Type 3 column)"]
    C -- No --> D[Skip]
    P --> T[(Single dim table\nmultiple rows per key\ncurrent values + prev_ columns)]
```
...
<div id="viz-scd-type6" class="ts-viz"></div>
