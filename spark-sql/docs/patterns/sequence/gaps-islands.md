...
??? success "Expected output"
    ...
    | user-C  | 2024-04-01   | NULL       | NULL          | active      |

    The final `user-C` row has `period_end = NULL` because there's no later event to
    pair it with — `LEAD()` returns `NULL` for the last row in each partition. The
    `WHERE period_end IS NOT NULL OR event_type = 'activated'` clause deliberately keeps
    this row instead of dropping it, so a **currently active** subscription (one still
    open as of the last known event) still shows up as an open-ended `active` period
...
### Scenario 7: Stock Availability / Out-of-Stock Detection

Given daily inventory snapshots, find the contiguous **in-stock** periods per SKU
(`on_hand_qty > 0`) and count how many separate stockout events each SKU had — a
short, frequent trickle of 1-day stockouts is a different supply-chain problem than
one long outage, even if total days-out-of-stock is the same.

```sql
CREATE OR REPLACE TEMP VIEW inventory_snapshot AS
SELECT * FROM VALUES
  ('SKU-100', DATE '2024-01-01', 42),
  ('SKU-100', DATE '2024-01-02', 30),
  ('SKU-100', DATE '2024-01-03', 12),
  ('SKU-100', DATE '2024-01-04',  0),  -- stockout begins
  ('SKU-100', DATE '2024-01-05',  0),
  ('SKU-100', DATE '2024-01-06', 25),  -- restocked
  ('SKU-100', DATE '2024-01-07', 18),
  ('SKU-100', DATE '2024-01-08',  0),  -- second, shorter stockout
  ('SKU-100', DATE '2024-01-09', 40),
  ('SKU-200', DATE '2024-01-01',  5),
  ('SKU-200', DATE '2024-01-02',  5),
  ('SKU-200', DATE '2024-01-03',  5)
AS t(sku, snapshot_date, on_hand_qty);
```
...
ranked AS (
    SELECT
        ...
        DATE_SUB(
            snapshot_date,
            ROW_NUMBER() OVER (PARTITION BY sku ORDER BY snapshot_date)
        ) AS island_anchor
    FROM in_stock_days
)
SELECT
    sku,
    MIN(snapshot_date)                                    AS in_stock_from,
    MAX(snapshot_date)                                    AS in_stock_to,
    DATEDIFF(MAX(snapshot_date), MIN(snapshot_date)) + 1 AS days_in_stock
FROM ranked
GROUP BY sku, island_anchor
ORDER BY sku, in_stock_from;
```

1. Filtering to `on_hand_qty > 0` *before* the `ROW_NUMBER()` — out-of-stock days are
    the gaps, so they must not appear in the ranked set at all, not merely be flagged.

??? success "Expected output"

    | sku     | in_stock_from | in_stock_to | days_in_stock |
    | ------- | ------------- | ----------- | ------------- |
    | SKU-100 | 2024-01-01    | 2024-01-03  | 3             |
    | SKU-100 | 2024-01-06    | 2024-01-07  | 2             |
    | SKU-100 | 2024-01-09    | 2024-01-09  | 1             |
    | SKU-200 | 2024-01-01    | 2024-01-03  | 3             |

Counting islands per SKU turns this into a stockout-frequency signal:
...
ranked AS (
    SELECT sku, snapshot_date,
        DATE_SUB(snapshot_date, ROW_NUMBER() OVER (PARTITION BY sku ORDER BY snapshot_date)) AS island_anchor
    FROM in_stock_days
)
SELECT sku, COUNT(*) AS in_stock_island_count
FROM (SELECT DISTINCT sku, island_anchor FROM ranked)
GROUP BY sku;
```

??? success "Expected output"

    | sku     | in_stock_island_count |
    | ------- | --------------------- |
    | SKU-100 | 3                     |
    | SKU-200 | 1                     |

`SKU-100` restocks constantly but keeps running out — 3 separate in-stock islands
in 9 days is a reorder-point problem, not just a total-quantity problem, and that
signal disappears the moment you aggregate `SUM(on_hand_qty)` instead of counting
islands.
...
| Stock availability / stockouts     | Filter to in-stock rows, then Date − ROW_NUMBER() | Out-of-stock days are the gaps, not a flagged state |
