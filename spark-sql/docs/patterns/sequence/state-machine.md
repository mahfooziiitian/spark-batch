...
## :material-cart-outline: Business Domain Example — Order Lifecycle

The infrastructure examples above (warehouses, clusters) generalize directly to
business entities. An **order lifecycle** — `CREATED → PAID → SHIPPED → DELIVERED → RETURNED` — needs the same four columns for every event:
`current_state`, `previous_state`, `invalid_transition`, and `time_in_state`.
Verified on Spark 4.2:

```sql
CREATE OR REPLACE TEMP VIEW order_events AS
SELECT * FROM VALUES
  ('ord_1', 'CREATED',   TIMESTAMP '2024-05-01 09:00:00'),
  ('ord_1', 'PAID',      TIMESTAMP '2024-05-01 09:05:00'),
  ('ord_1', 'SHIPPED',   TIMESTAMP '2024-05-02 14:00:00'),
  ('ord_1', 'DELIVERED', TIMESTAMP '2024-05-04 11:00:00'),
  ('ord_2', 'CREATED',   TIMESTAMP '2024-05-01 10:00:00'),
  ('ord_2', 'PAID',      TIMESTAMP '2024-05-01 10:02:00'),
  ('ord_2', 'DELIVERED', TIMESTAMP '2024-05-03 09:00:00'),  -- invalid: skipped SHIPPED
  ('ord_3', 'CREATED',   TIMESTAMP '2024-05-01 08:00:00'),
  ('ord_3', 'PAID',      TIMESTAMP '2024-05-01 08:10:00'),
  ('ord_3', 'SHIPPED',   TIMESTAMP '2024-05-02 09:00:00'),
  ('ord_3', 'DELIVERED', TIMESTAMP '2024-05-03 09:00:00'),
  ('ord_3', 'RETURNED',  TIMESTAMP '2024-05-10 09:00:00')
AS t(order_id, state, event_time);

CREATE OR REPLACE TEMP VIEW valid_order_transitions AS
SELECT * FROM VALUES
  ('CREATED',   'PAID'),
  ('PAID',      'SHIPPED'),
  ('SHIPPED',   'DELIVERED'),
...
WITH enriched AS (
    SELECT
        ...
        LAG(state) OVER (PARTITION BY order_id ORDER BY event_time)  AS previous_state,
        event_time,
        LEAD(event_time) OVER (PARTITION BY order_id ORDER BY event_time) AS next_event_time
    FROM order_events
)
SELECT
    e.order_id,
    ...
    e.event_time,
    CASE
        WHEN e.previous_state IS NULL THEN false  -- (1)!
        WHEN v.from_state IS NULL THEN true        -- (2)!
        ELSE false
    END AS invalid_transition,
    CAST(e.next_event_time AS LONG) - CAST(e.event_time AS LONG) AS time_in_state_seconds -- (3)!
FROM enriched e
LEFT JOIN valid_order_transitions v
    ON e.previous_state = v.from_state AND e.current_state = v.to_state
ORDER BY e.order_id, e.event_time;
```

1. The first event per order has no previous state — that's a new order, not an
    invalid transition.
2. A `LEFT JOIN` miss means the `(previous_state, current_state)` pair is not in
    the allowed-transitions table.
3. `NULL` on the most recent event per order means "still in that state as of
    now" — not missing data.

??? success "Expected output"

    | order_id | previous_state | current_state | event_time          | invalid_transition | time_in_state_seconds |
    | -------- | -------------- | ------------- | ------------------- | ------------------ | --------------------- |
    | ord_1    | NULL           | CREATED       | 2024-05-01 09:00:00 | false              | 300                   |
    | ord_1    | CREATED        | PAID          | 2024-05-01 09:05:00 | false              | 104100                |
    | ord_1    | PAID           | SHIPPED       | 2024-05-02 14:00:00 | false              | 162000                |
    | ord_1    | SHIPPED        | DELIVERED     | 2024-05-04 11:00:00 | false              | NULL                  |
    | ord_2    | NULL           | CREATED       | 2024-05-01 10:00:00 | false              | 120                   |
    | ord_2    | CREATED        | PAID          | 2024-05-01 10:02:00 | false              | 169080                |
    | ord_2    | PAID           | DELIVERED     | 2024-05-03 09:00:00 | **true**           | NULL                  |
    | ord_3    | NULL           | CREATED       | 2024-05-01 08:00:00 | false              | 600                   |
    | ord_3    | CREATED        | PAID          | 2024-05-01 08:10:00 | false              | 89400                 |
    | ord_3    | PAID           | SHIPPED       | 2024-05-02 09:00:00 | false              | 86400                 |
    | ord_3    | SHIPPED        | DELIVERED     | 2024-05-03 09:00:00 | false              | 604800                |
    | ord_3    | DELIVERED      | RETURNED      | 2024-05-10 09:00:00 | false              | NULL                  |

    `ord_2` skips `SHIPPED` entirely (`PAID → DELIVERED`) and is correctly flagged
    `invalid_transition = true` — the same `LAG`/`LEFT JOIN` logic that catches
    infrastructure state-skips catches business-process state-skips.

!!! tip "The same four columns work across domains"

    `current_state` / `previous_state` / `invalid_transition` / `time_in_state`
    is a domain-agnostic template. Only the state vocabulary and the
    `valid_transitions` table change:

    | Domain                    | States                                                   | `time_in_state` answers                              |
    | ------------------------- | -------------------------------------------------------- | ---------------------------------------------------- |
    | Orders                    | `CREATED → PAID → SHIPPED → DELIVERED → RETURNED`        | How long between payment and shipment?               |
    | Payments                  | `INITIATED → AUTHORIZED → CAPTURED → SETTLED → REFUNDED` | How long stuck in `AUTHORIZED`?                      |
    | Workflows / job execution | `QUEUED → RUNNING → SUCCEEDED`/`FAILED → RETRYING`       | How long did the job wait in `QUEUED`?               |
    | IoT devices               | `PROVISIONED → ONLINE → DEGRADED → OFFLINE`              | How long was the device `DEGRADED` before failing?   |
    | Customer lifecycle        | `LEAD → TRIAL → ACTIVE → CHURNED → REACTIVATED`          | How long from `TRIAL` to `ACTIVE` (conversion time)? |
...
| Payment processing          | Stuck in `AUTHORIZED`, invalid capture/refund order            |
| Job/workflow execution      | Retry loops, stuck in `QUEUED`                                 |
| Customer lifecycle          | Trial-to-active conversion time, unexpected churn→reactivation |
...
- [Event Stream Analytics](event-stream-analytics.md) — general event processing
- [Gaps & Islands](gaps-islands.md) — consecutive state duration calculation
