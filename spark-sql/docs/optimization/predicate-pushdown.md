# :material-filter-variant: Predicate Pushdown
...
Toggle between a direct column filter and a UDF-wrapped filter to see the difference in
`PushedFilters`, confirmed against a real PySpark 4.2 `EXPLAIN FORMATTED` run.
