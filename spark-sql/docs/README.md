...
Full architecture, setup, and usage guide: [`mcp-server.md`](../../docs/mcp-server.md)
...
the SQL Warehouses API (`../src/spark_sql/dbx_mcp/tools/warehouses.py`), so only one of the two is required.

## Environments (`APP_ENV`)

The server resolves `dev`/`staging`/`prod` the same way `make dab-*` does: `../src/spark_sql/dbx_mcp/config/settings.py`
loads `.env` (base) then layers `.env.$(APP_ENV)` on top (see `dashboard.util.cli_env.load_env_layers`),
defaulting to `dev` when `APP_ENV` is unset. `settings.app_env` exposes the resolved value.
...
From your editor, `../.mcp.json` forwards `APP_ENV` (`${env:APP_ENV}`) — export it in the
...
Two mutually exclusive options, resolved in `../src/spark_sql/dbx_mcp/clients/databricks_client.py`:
...
| Symptom | Fix |
|---------|-----|
| `~/.databrickscfg has no <profile> profile configured` | Run `databricks auth login --host <workspace-url> --profile <profile>` |
| `Neither databricks_warehouse_id nor databricks_warehouse_name is configured` | Set `DATABRICKS_WAREHOUSE_ID` or `DATABRICKS_WAREHOUSE_NAME` in `../.env` |
| Copilot Chat shows the server disconnected | Reload the VS Code window; confirm no tool logs to stdout |

See [`mcp-server.md`](../../docs/mcp-server.md) for the full architecture, troubleshooting, and safety-model writeup.
