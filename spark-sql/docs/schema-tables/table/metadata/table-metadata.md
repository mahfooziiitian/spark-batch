...
## :material-database-search: [Databricks] Real-World: Governance Metadata via System Tables

`DESCRIBE`/`SHOW` reflect a table's *current* state. For **historical** metadata
— who reads it, what it costs, when it was last modified — query the
Unity Catalog `system` catalog instead.

```sql
-- Combine live schema with tag-based classification
DESCRIBE TABLE EXTENDED analytics.silver.customers;
...
-- Storage/compute cost tied to jobs that write this table
SELECT usage_metadata.job_id, usage_date, SUM(usage_quantity) AS dbu_usage
FROM system.billing.usage
WHERE custom_tags['table'] = 'analytics.silver.customers'
GROUP BY usage_metadata.job_id, usage_date
ORDER BY usage_date DESC;

-- Who has queried this table in the last 30 days?
SELECT user_identity.email AS actor, COUNT(*) AS query_count
FROM system.access.audit
WHERE service_name = 'unityCatalog'
  AND action_name = 'getTable'
  AND request_params['full_name_arg'] = 'analytics.silver.customers'
  AND event_time >= current_date() - INTERVAL 30 DAYS
GROUP BY user_identity.email
ORDER BY query_count DESC;
```

!!! note "[Databricks] system catalog required"

    These queries require Unity Catalog with system tables enabled
    (`SELECT` on `system.information_schema`, `system.billing`, `system.access`).
    See [Unity Catalog](../../../data-sources/catalog/unity.md) for setup details.
