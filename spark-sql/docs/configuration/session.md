...
### :material-animation-play: Interactive Visualization — Session Scope in Action

Toggle between `SET`, `SET -v`, `RESET key`, and `RESET` to see which values change in the
current session and which stay untouched elsewhere. The example contrasts two sessions
against the same cluster default.

<div id="viz-config-session" class="ts-viz"></div>
...
4. `SET -v` returns every known Spark and Hadoop configuration key, its current value, and its meaning — output can be thousands of rows.
5. **Static settings** (for example, `spark.executor.memory`, `spark.executor.cores`) cannot be changed with `SET` after the SparkContext starts.
6. Custom user-defined properties can be stored with `SET` and read back with `SET key`; whether they participate in SQL text substitution depends on the client or runtime, not just the SparkSession itself.
...
`SET -v` is primarily an inspection tool. In notebooks and SQL editors, filter its result
grid by key prefix such as `spark.sql.` or `spark.hadoop.` rather than scrolling the full
output every time.

### Store a custom session property

```sql
SET my.pipeline.run_date = 2024-06-01;
SET my.pipeline.run_date;
```

This is useful for lightweight metadata or client-side parameterization, but do not assume
every engine will substitute `${my.pipeline.run_date}` inside SQL text automatically.
...
| Check the current effective value        | `SET key`                               |
| Bulk-reset after a tuning session        | `RESET`                                 |
| Audit all active settings                | `SET -v`                                |
| Store a temporary session property       | `SET my.key = value`                    |

!!! tip "Always RESET after per-query tuning"

    In long-running sessions (notebooks, interactive SQL editors), always `RESET` after
    per-query overrides. Leaving `shuffle.partitions = 10` in place will silently
    under-parallelise later queries in the same session.
