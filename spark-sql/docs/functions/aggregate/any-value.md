...
### :material-animation-play: Interactive Visualization — Non-Deterministic Across Runs

<div id="viz-any-value-runs" class="ts-viz"></div>

Click "run again" to simulate repeated executions of the same
`ANY_VALUE(rep)` query — the returned value can change between runs on the
same data, unlike `MIN_BY`/`MAX_BY`, which are always reproducible.
