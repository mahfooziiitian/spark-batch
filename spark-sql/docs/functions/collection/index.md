...
### :material-animation-play: Interactive Visualization — Collection Capability Matrix

<div id="viz-collection-capabilities" class="ts-viz"></div>

Select a capability to see which collection types actually support it in
Spark SQL. `MAP` is the outlier: no ordering, no `=` equality, and it can't
participate in `DISTINCT`/set operations at all.

______________________________________________________________________

## :material-alert-outline: Behavior Gotchas & Limitations

1. **`MAP` has no ordering or equality operator.** `MAP(1,'a') = MAP(1,'a')` fails to resolve — `[DATATYPE_MISMATCH.INVALID_ORDERING_TYPE]` — because map key order is unspecified, so Spark can't define a canonical comparison. `ARRAY` and `STRUCT` both support `=`.
2. **`MAP` cannot be used in `DISTINCT`, `INTERSECT`, `EXCEPT`, or `GROUP BY`.** `SELECT DISTINCT map_col FROM t` fails with `[UNSUPPORTED_FEATURE.SET_OPERATION_ON_MAP_TYPE]`. Workaround: convert to a sorted array of entries first — `ARRAY_SORT(MAP_ENTRIES(m))` — which *is* comparable/orderable, then dedupe/group on that.
3. **`STRUCT` equality is field-by-field and NULL-safe per field**, not standard three-valued NULL logic: `NAMED_STRUCT('a',1,'b',CAST(NULL AS INT)) = NAMED_STRUCT('a',1,'b',CAST(NULL AS INT))` returns `true` — two structs with matching NULL fields are considered equal, unlike scalar `NULL = NULL`, which is `NULL`.
4. **`ARRAY_SORT` and `SORT_ARRAY` place NULLs differently by default** — verified on Spark 4.2: `SORT_ARRAY(arr, true)` (ascending, the default) puts NULLs **first**, but `ARRAY_SORT(arr)` puts NULLs **last** regardless of direction. See [array.md](array.md) for the full comparison and how to override it with a custom comparator.
...
SELECT NAMED_STRUCT('a',1,'b',CAST(NULL AS INT)) = NAMED_STRUCT('a',1,'b',CAST(NULL AS INT));
-- Result: true
```
