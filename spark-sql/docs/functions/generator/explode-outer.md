...
<div id="viz-explode-outer" class="ts-viz"></div>

Click an input row to trace it. Unlike plain `EXPLODE`, the row with a `NULL`/empty array is
**not** dropped — it produces exactly one output row with `col = NULL`.
...
6. **Semantically equivalent to a `LEFT OUTER JOIN`** between the base rows and their exploded elements — `EXPLODE` is the `INNER JOIN`-like variant that drops unmatched (NULL/empty) rows.
7. **Safe nested field access** — accessing a field on a `NULL` struct (e.g. `item.qty` when `item IS NULL`) returns `NULL` rather than erroring, so `INLINE_OUTER`/`EXPLODE_OUTER` + dot-notation naturally propagate `NULL`s without extra `COALESCE` guards.
...
### :material-toy-brick: 6. Struct Field Access Stays NULL-Safe

```sql
CREATE OR REPLACE TEMP VIEW orders_nested AS
SELECT * FROM VALUES
  (1, ARRAY(NAMED_STRUCT('product', 'pen', 'qty', 3))),
  (2, NULL)
AS orders_nested(order_id, items);

-- item is NULL for order 2; item.product / item.qty resolve to NULL automatically
SELECT order_id, item.product, item.qty
FROM orders_nested
LATERAL VIEW EXPLODE_OUTER(items) AS item;
-- (1, pen, 3), (2, NULL, NULL)
