...
### :material-toy-brick: 5. Deriving Array Length from `pos` (and Why It Needs a Guard)

```sql
-- MAX(pos) + 1 recovers the array length per group WITHOUT calling SIZE()
-- but empty/NULL arrays yield pos = NULL, so MAX(pos) is NULL too — guard with COALESCE
SELECT device_id,
       COALESCE(MAX(pos) + 1, 0) AS num_readings
FROM sensor_log
LATERAL VIEW POSEXPLODE_OUTER(readings) AS pos, reading
GROUP BY device_id;
-- (dev01, 2), (dev02, 0), (dev03, 0)
-- Prefer SIZE(readings) directly when you only need the count and not the rows.
```
