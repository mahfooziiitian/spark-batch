...
def test_first_value_per_partition(spark: SparkSession, sales_view: str):
    ...
    alice_north = result.filter((F.col("region") == "North") & (F.col("rep") == "Alice")).collect()
    assert all(row.first_sale == 100 for row in alice_north), "FIRST_VALUE should be 100 for all North/Alice rows"
...
def test_last_value_with_full_frame(spark: SparkSession, sales_view: str):
    ...
    alice_north = result.filter((F.col("region") == "North") & (F.col("rep") == "Alice")).collect()
    assert all(row.last_sale == 300 for row in alice_north), "LAST_VALUE (full frame) should be 300 for all North/Alice rows"
...
def test_nth_value(spark: SparkSession, sales_view: str):
    ...
    alice_north = result.filter((F.col("region") == "North") & (F.col("rep") == "Alice")).collect()
    assert all(row.second_sale == 200 for row in alice_north), "NTH_VALUE(2) should be 200 for all North/Alice rows"
