"""Validates SQL semantics from src/scd/type2/scd_type2.sql. Delta MERGE is emulated with temp-view SQL."""
...
class TestSCD2:
    ...
    def _register_views(cls, spark: SparkSession) -> None:
        create_view(
            ...
            "dim_customer",
            [
                (1, "Alice", "alice@example.com", "New York", date(2023, 1, 1), None, True),
                (2, "Bob", "bob@example.com", "Chicago", date(2023, 1, 1), None, True),
                (3, "Carol", "carol@example.com", "Austin", date(2023, 1, 1), None, True),
            ],
            cls._target_schema(),
        )
        create_view(
    ...
    def test_versions_changed_and_new_rows(self: TestSCD2, spark: SparkSession) -> None:
        assert_query_in_source("src/scd/type2/scd_type2.sql", STEP_ONE_FRAGMENT)
        self._register_views(spark)

        actual = self._run_merge(spark)
        expected = spark.createDataFrame(
            [
                (1, "Alice", "alice@example.com", "New York", date(2023, 1, 1), date(2024, 6, 15), False),
                (1, "Alice", "alice@newemail.com", "Boston", date(2024, 6, 15), None, True),
                (2, "Bob", "bob@example.com", "Chicago", date(2023, 1, 1), None, True),
                (3, "Carol", "carol@example.com", "Austin", date(2023, 1, 1), None, True),
                (4, "Dave", "dave@example.com", "Seattle", date(2024, 6, 15), None, True),
            ],
