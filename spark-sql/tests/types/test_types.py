...
class TestTypes:
    def test_array_query_comes_from_source(
        self: TestTypes, spark: SparkSession
    ) -> None:
        execute_sql_file(
            spark,
        ...
        statement = statement_containing(
            "sql/types/array/array.sql", "WHERE ARRAY_CONTAINS(tags, 'gift')"
        )
    ...
    def test_map_and_struct_queries_execute_from_source(
        self: TestTypes, spark: SparkSession
    ) -> None:
        ...
        struct_filter = statement_containing(
            "sql/types/struct/struct.sql", "WHERE address.country = 'USA'"
        )
        struct_explode = statement_containing(
            "sql/types/struct/struct.sql", "LATERAL VIEW INLINE(line_items)"
        )
    ...
    def test_datetime_query_comes_from_source(
        self: TestTypes, spark: SparkSession
    ) -> None:
        execute_sql_file(
            spark,
