...
def join_on_skewed_data_with_sub_split(
    spark: SparkSession, data_file: str, lookup_data_file: str
):
    sub_split_partitions = 20
    trips_data = (
        prepare_trips_data(spark=spark, data_file=data_file)
...
def prepare_trips_data(spark: SparkSession, data_file: str) -> DataFrame:
    ...
    res_df = spark.read.parquet(data_file).withColumn(
        "PULocationID",
...
def main():
    lookup_data_file = (
        f"{os.environ['DATA_HOME']}/FileData/Csv/trips/taxi_zone_lookup.csv"
    )
    ...
    print(
        f"Hadoop version = {spark.sparkContext.jvm.org.apache.hadoop.util.VersionInfo.getVersion()}"
    )

    join_on_skewed_data_with_sub_split(
        spark=spark, data_file=trip_data_file, lookup_data_file=lookup_data_file
    )
