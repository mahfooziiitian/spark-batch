...
if __name__ == "__main__":
    ...
    click_stream_data = (
        ...
        + "/FileData/Text/click_stream_data.txt"
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            ...
            | (F.col("time_diff") > session_expire_time * 60),
    ...
    df = df.withColumn(
        "session_id", F.last("session_id", ignorenulls=True).over(window_spec)
    )
    ...
    df = df.withColumn(
        "session_start_time",
        F.min("Timestamp").over(Window().partitionBy("user_id", "session_id")),
    )
    df = df.withColumn(
        "session_end_time",
        F.max("Timestamp").over(Window().partitionBy("user_id", "session_id")),
    )
    ...
    daily_time_spent = (
        df.groupBy(F.to_date("Timestamp").alias("date"), "user_id")
        .agg(F.countDistinct("session_id").alias("num_sessions"))
        .agg(F.sum("session_duration").alias("total_time_spent"))
    )
