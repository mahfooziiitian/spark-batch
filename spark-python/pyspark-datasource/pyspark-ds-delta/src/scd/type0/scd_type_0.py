...
def get_sample(spark: SparkSession, url: str):
    ...
    df.createOrReplaceTempView("EmployeeSample")
...
def main():
    ...
    get_sample(spark, url)

    spark.sql("SELECT * FROM EmployeeSample")

    spark.sql("CREATE DATABASE IF NOT EXISTS scd")
    spark.sql("DROP TABLE IF EXISTS scd.employees")

    # Create new table (delta format) using the sample Employee dataset
    spark.sql("CREATE TABLE scd.employees USING delta AS SELECT * FROM EmployeeSample")

    spark.sql("SELECT * FROM scd.employees").show(truncate=False)
