...
if __name__ == "__main__":
    ...
    DeltaTable.createIfNotExists(spark).tableName(
        "default.python_data_table_builder"
    ...
    ).addColumn("gender", "STRING").addColumn("birthDate", "TIMESTAMP").addColumn(
        "ssn", "STRING"
    ).addColumn("salary", "INT").property(
        "description", "table with people data"
    ).location(
        Path(f"{data_home}/FileData/Delta/crud/python_data_table_builder").as_posix()
    ).execute()
