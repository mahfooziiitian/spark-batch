...
if __name__ == "__main__":
    ...
    delta_table = DeltaTable.forPath(
        spark, f"{data_home}/FileData/Delta/partition/partition-table"
    )
    ...
    delta_table.alias("old_data").merge(
        update_df.alias("new_data"), "old_data.name = new_data.name"
    ).whenMatchedUpdateAll().execute()

    # Read the updated Delta table
    updated_df = spark.read.format("delta").load(
        f"{data_home}/FileData/Delta/partition/partition-table"
    )
    updated_df.show()
