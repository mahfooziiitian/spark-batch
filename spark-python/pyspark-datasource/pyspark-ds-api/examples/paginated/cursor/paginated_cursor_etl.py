"""Cursor-pagination ETL demo.

Pairs with ``cursor_pagination_source.py`` (mock server on port 8081):

    PYTHONPATH=src uv run python examples/paginated/cursor/cursor_pagination_source.py &
    PYTHONPATH=src uv run python examples/paginated/cursor/paginated_cursor_etl.py
"""
...
DEFAULT_CONFIG = Path(__file__).parent / "paginated_cursor_etl.yaml"
...
def main() -> int:
    ...
    try:
        ...
        logger.info("Extracting data from API (cursor pagination)...")
