"""Centralized logging configuration for the pys_html library.

Provides a pre-configured logger with Rich-powered formatting and level control
via the PYS_HTML_LOG_LEVEL environment variable.
...
# Custom theme for pys_html output
_THEME = Theme(
    {
        "info": "cyan",
        "warning": "yellow",
...
def _configure_root_logger() -> None:
    """One-time setup of the pys_html logger hierarchy with Rich handler."""
    global _configured
    if _configured:
        return

    level_name = os.environ.get("PYS_HTML_LOG_LEVEL", "INFO").upper()
    level = getattr(logging, level_name, logging.INFO)

    root_logger = logging.getLogger("pys_html")
...
def get_logger(name: str) -> logging.Logger:
    """Get a child logger under the pys_html namespace.
    ...
    return logging.getLogger(f"pys_html.{name}")


def set_log_level(level: str | int) -> None:
    """Dynamically change the log level for all pys_html loggers.
...
def print_dataframe(df, title: str = "DataFrame", max_rows: int = 20) -> None:
    ...
    for field in df.schema.fields:
        table.add_column(field.name, style="cyan", overflow="fold", ratio=1)
