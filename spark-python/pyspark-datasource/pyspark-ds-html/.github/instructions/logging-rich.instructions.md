---
applyTo: "{**/examples/**/*.py,src/pys_html/_logging.py}"
---

# Logging & Rich Output Instructions

## Library Logging (`pys_html._logging`)

The library provides a centralized logging system powered by **Rich**.

### Logger hierarchy

All loggers live under the `pys_html.*` namespace:

```python
from pys_html._logging import get_logger

logger = get_logger("example.my_script")  # → pys_html.example.my_script
```
...
from pys_html import set_log_level
