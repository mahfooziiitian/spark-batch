"""Credit-evaluation feature extraction from namespaced XML using Spark XPath.

Parses Experian ``RespNewCreditEvaluation`` responses (with an ``ns0`` default
namespace that Spark strips automatically) and derives risk-model features with
``xpath_string`` / ``xpath_boolean``, then applies conditional scoring logic in
a CTE. Self-contained — sample responses are embedded inline. Run it directly:
...
if __name__ == "__main__":
    spark = SparkSession.builder.appName("xml_xpath").master(os.environ.get("SPARK_MASTER", "local[*]")).getOrCreate()
    ...
    # Shared XPath prefixes (Spark strips the ns0 default namespace automatically).
    base = "RespNewCreditEvaluation/InboundResponse"
    dv_app = f"{base}/DV-Application"
    dv_rst = f"{base}/DV-Results/RST[@cxArrayIndex=1]"
    equ = f"{base}/BureauResult/EQU/RiskModel[@cxArrayIndex=1]"
    exp = f"{base}/BureauResult/EXP/RiskModel[@cxArrayIndex=1]"

    # Compose the feature-extraction SELECT list from the reusable prefixes.
    select_exprs = ",\n            ".join(
        [
            "data",
            f"xpath_string(data, '{dv_app}/ApplicationCreationDate') as INQUIRY_DATE",
            f"xpath_string(data, '{dv_app}/ApplicationStatus') as REPORT_TYPE",
            f"xpath_string(data, '{dv_rst}/LiabilityIndicator') as LIABILITY_TYPES",
            f"xpath_string(data, '{equ}/ModelID') as eqfRiskModelId",
            f"xpath_string(data, '{equ}/Score') as eqfScore",
            f"xpath_string(data, '{equ}/ModelNumber') as eqfModelNumber",
            f"xpath_string(data, '{equ}/ReasonCode1') as eqfReasonCode1",
            f"xpath_string(data, '{equ}/ReasonCode2') as eqfReasonCode2",
            f"xpath_string(data, '{equ}/ReasonCode3') as eqfReasonCode3",
            f"xpath_string(data, '{equ}/ReasonCode4') as eqfReasonCode4",
            f"xpath_string(data, '{exp}/ScoreID') as evsRiskModel",
            f"xpath_boolean(data, '{equ}[ ModelID >= 2 and ModelID <= 4 ]') as isEqfRiskModel",
            f"xpath_string(data, '{exp}/ScoreID') = 'EV' as isEvsRiskModel",
            f"xpath_string(data, '{exp}/Score') as Score",
            f"xpath_string(data, '{exp}/ScoreID') = 'TC' as isTecRiskModel",
        ]
    )
    ...
    query = f"""
        ...
        SELECT *,
            ...
            isTecRiskModel
                AND (
                    instr(Score, '9') = 1
                    OR (try_cast(Score AS int) >= 0 AND try_cast(Score AS int) < 1000)
