...
def generate_element_data(xsd_element):
    ...
    if xsd_element.type.is_simple():
        ...
        elem.text = generate_random_value(xsd_element.type.prefixed_name)

    # Check if it's a complex type with child elements
    elif xsd_element.type.is_complex():
        # content holds the sequence/group definition of the child elements
        ...
        if content:
            for child_xsd_element in content.elements:
                ...
                # xmlschema represents "unbounded" as None; cap it at a reasonable bound
                max_occurs = child_xsd_element.max_occurs if child_xsd_element.max_occurs is not None else 3
...
def main():
    ...
    output_file = str(data_home / "file_data" / "xml" / "orders_catalog.xml")
    ...
    try:
        ...
        # 2. Get the root ('Orders') element definition from the schema.
        # NOTE: schema.root is the raw <xs:schema> ElementTree node, not the data
        # root element -- use schema.elements[...] to get the XsdElement instead.
        root_element_name = "Orders"
        root_element_schema = schema.elements[root_element_name]
        xml_root = ET.Element(root_element_name)

        # 3. Find the 'Order' element definition within the root's content type
        order_element_schema = next(e for e in root_element_schema.type.content.elements if e.name == "Order")
        ...
        print(f"Successfully generated {num_orders} orders and saved to '{output_file}'.")
