...
Enterprise-style `SparkSession` factory: resolves the master (`SPARK_MASTER`
env var, else `local[*]`), creates the warehouse directory, enables adaptive
query execution, and sets the log level. On Spark 4.0+ the XML data source is
built in, so no JAR configuration is required; `scala_version` /
`spark_xml_version` are only applied when `use_external_spark_xml_jar=True`
(for backward compatibility with Spark 3.x).

```python
from spark_xml.util.session.spark_session_util import get_spark_session

spark = get_spark_session(app_name="my-app")
```
...
| `master` | `str \| None` | `SPARK_MASTER` env var, else `local[*]` | Spark master URL |
| `warehouse_dir` | `str \| None` | `SPARK_WAREHOUSE` env var, else `/tmp/spark-warehouse` | `spark.sql.warehouse.dir` (created if missing) |
| `log_level` | `str` | `"WARN"` | Passed to `sparkContext.setLogLevel` |
| `use_external_spark_xml_jar` | `bool` | `False` | Add the Databricks `spark-xml` package (Spark < 4.0 only) |
| `scala_version` | `str` | `"2.12"` | Scala version for the spark-xml JAR |
| `spark_xml_version` | `str` | `"0.17.0"` | spark-xml library version |
| `extra_conf` | `Mapping[str, str] \| None` | `None` | Extra Spark configuration overrides |

**Returns:** `SparkSession`

See also: [Enterprise Patterns](../guide/enterprise.md).
...
> **Source:** `src/spark_xml/util/validate_xml_xsd.py`
...
> **Source:** `src/spark_xml/util/generate_xml_data.py`
...
> **Source:** `src/spark_xml/util/xml_to_xsd.py`
...
> **Source:** `src/spark_xml/util/xml_to_xsd_trang.py`
