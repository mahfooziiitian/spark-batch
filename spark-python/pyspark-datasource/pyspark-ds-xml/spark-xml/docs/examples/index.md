...
Each example script under `examples/` is self-contained and executable:

```bash
uv run python examples/<feature>/<script>.py
```
...
| Script | Description |
|---|---|
| `enterprise/etl_pipeline_orders.py` | Production-style ingestion job: shared `SparkSession` factory, structured logging, enforced schema, corrupt-record quarantine, and a partitioned Parquet sink. See [Enterprise Patterns](../guide/enterprise.md). |
...
# Namespace handling
uv run python examples/namespace/ignore_namespace_xml.py
