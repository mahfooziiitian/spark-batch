"""Centralized logging configuration for the pys_json library.

Provides a pre-configured logger with Rich-powered formatting and level control
via the PYS_JSON_LOG_LEVEL environment variable.
...
# Custom theme for pys_json output
_THEME = Theme(
    {
        "info": "cyan",
        "warning": "yellow",
...
def _configure_root_logger() -> None:
    """One-time setup of the pys_json logger hierarchy with Rich handler."""
    global _configured
    if _configured:
        return

    level_name = os.environ.get("PYS_JSON_LOG_LEVEL", "INFO").upper()
    level = getattr(logging, level_name, logging.INFO)

    root_logger = logging.getLogger("pys_json")
...
def get_logger(name: str) -> logging.Logger:
    """Get a child logger under the pys_json namespace.
    ...
    return logging.getLogger(f"pys_json.{name}")


def set_log_level(level: str | int) -> None:
    """Dynamically change the log level for all pys_json loggers.
